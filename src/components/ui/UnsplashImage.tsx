...
import { getPhoto, sized } from "@/lib/utils";
...
/**
 * next/image wrapper that pulls from our curated images.json and renders
 * the photographer attribution Unsplash's API Guidelines require.
 */
    ...
    <figure className={cn("relative overflow-hidden", className)}>
      ...
      {showCredit && photo.credit.name && (
        <figcaption className="pointer-events-auto absolute bottom-2 right-2 z-10 rounded-full bg-black/55 px-2.5 py-1 text-[10px] text-white/70 backdrop-blur-sm">
          Photo:{" "}
          <a
            href={`${photo.credit.link}?utm_source=marauders_portfolio&utm_medium=referral`}
            target="_blank"
